...
# if TYPE_CHECKING:
#     from _operator import _K, _P, _R, _SupportsComparison
...
from typing import TypeVar, runtime_checkable, Protocol,AbstractSet,ItemsView as _ItemsView,ValuesView as _ValuesView,KeysView as _KeysView,Any
_T_co = TypeVar("_T_co", covariant=True)
_KT_co = TypeVar("_KT_co", covariant=True)
_VT_co = TypeVar("_VT_co", covariant=True)
_KT = TypeVar("_KT")
...
# # Combined protocol inherits from base protocols
# class SupportsGetItemBuffer(Protocol):
#     def __contains__(self, x: Any, /) -> bool: ...
#     @overload
#     def __getitem__(self, slice: slice, /) -> Sequence[int]: ...
#     @overload
...
# _BufferWithLen: TypeAlias = SizedBuffer
...
#                 self.data[i]
#                 if isinstance(i, str)
#                 else islice_extended(cast(DictLike, self.data).items(), i.start, i.stop, i.step)
...
# from typing import TypeVarTuple, NamedTuple, Tuple
# Ts = TypeVarTuple("Ts")
# Us = TypeVarTuple("Us")
# T = TypeVar("T")
# U = TypeVar("U")
...
# @runtime_checkable
# class CoordsLike(Protocol[Unpack[Ts]]):
