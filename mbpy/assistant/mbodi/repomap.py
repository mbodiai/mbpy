...
from mbodi.dump import dump
from mbodi.special import filter_important_files
from mbodi.utils import Spinner
...
class RepoMap:
    CACHE_VERSION = 3
    TAGS_CACHE_DIR = f".mbodi.tags.cache.v{CACHE_VERSION}"
    ...
    def get_repo_map(
        ...
        repo_content = self.repo_content_prefix.format(other=other) if self.repo_content_prefix else ""
    ...
    def get_ranked_tags(
        ...
        pers_args = {"personalization": personalization, "dangling": personalization} if personalization else {}
...
def find_src_files(directory):
    ...
    src_files = []
    for root, _, files in Path(directory).walk(directory):
        src_files.extend([Path(root) / file for file in files])
    return src_files


def get_random_color():
    hue = random.random() # noqa
