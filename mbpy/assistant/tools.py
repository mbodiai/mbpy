        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "code": {
                        "type": "string",
                        "description": "The code blocks to execute" "and return non-code parts",
                    },
                },
                "required": ["code"],
            },
...
def run_shell_command(command: str) -> ToolOutput:
    try:
        output = run(command)
        return {
            "output": output,
...
def pip_install(package: str) -> ToolOutput:
    try:
...
def dispatch_coding_assistant(
    instruction: str,
    subtask1: str | None = None,
    subtask2: str | None = None,
    subtask3: str | None = None,
    subtask4: str | None = None,
) -> ToolOutput:
    try:
        subtasks = [
            {
        ...
        return {
            "output": "\n".join(
                [f"Subtask {i+1}: {subtask['instruction']}\n{subtask['result']}" for i, subtask in enumerate(subtasks)]
            ),
        }
    except Exception as e:
        return {
