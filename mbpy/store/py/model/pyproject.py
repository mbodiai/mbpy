...
class Tox(BaseModel):
    ...
    legacy_tox_ini: str | None = Field(None, title='Tox configuration in ini format')
...
class BuildFrontend1(BaseModel):
    """Set the tool to use to build, either "pip" (default for now), "build", or "build[uv]"."""
...
class ContainerEngine1(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    name: Name1
...
class BuildFrontend3(BaseModel):
    """Set the tool to use to build, either "pip" (default for now), "build", or "build[uv]"."""
...
class ContainerEngine3(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    name: Name3
...
class PartialSetuptoolsScm(BaseModel):
...
class WheelDataArray1(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
...
class ConfigSettingValue1(BaseModel):
...
class ConfigSettingValue2(BaseModel):
...
class ToolUvWorkspace(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
...
class ProjectAuthor1(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    name: Any
    email: EmailStr | None = Field(None, title='Author email')


class ProjectAuthor2(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
...
class Project(BaseModel):
    ...
    name: constr(pattern=r'^([a-zA-Z\d]|[a-zA-Z\d][\w.-]*[a-zA-Z\d])$') = Field(
        ..., title='Project name',
    )
    version: constr(pattern='^v?((([0-9]+)!)?([0-9]+(\\.[0-9]+)*)([-_\\.]?(alpha|a|beta|b|preview|pre|c|rc)[-_\\.]?([0-9]+)?)?((-([0-9]+))|([-_\\.]?(post|rev|r)[-_\\.]?([0-9]+)?))?([-_\\.]?(dev)[-_\\.]?([0-9]+)?)?)(\\+([a-z0-9]+([-_\\.][a-z0-9]+)*))?$') | None = Field(None, examples=['42.0.1', '0.3.9rc7.post0.dev5'], title='Project version')
...
class PartialCibuildwheel(BaseModel):
...
class PartialScikitBuild(BaseModel):
...
class PartialSetuptools(BaseModel):
...
class DevDependencies(BaseModel):
...
class PartialPyright(BaseModel):
...
class PyProject(BaseModel):
    ...
    dependency_groups: Dict[constr(pattern='^([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9._-]*[a-zA-Z0-9])$'), List[str | DependencyGroups]] | None = Field(None, alias='dependency-groups', title='PEP 735 dependency groups')
