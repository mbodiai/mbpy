...
def make_decorator(deco, dargs=(), dkwargs=None):
    if dkwargs is None:
        dkwargs = {}
    @wraps(deco)
    def _decorator(func):
...
class Call:
    ...
    def __str__(self):
        ...
        args = ", ".join(list(map(str, self._args)) + ["{}={}".format(*t) for t in self._kwargs.items()])
        return f"{func}({args})"
...
def arggetter(func, _cache: MethodAttrCache = None):
    if _cache is None:
        _cache = {}
    if func in _cache:
        return _cache[func]
...
def reduce(function: Callable[[_T, _T], _T], sequence: Iterable[Callable[...,_T]], initial: Any | MissingT = _initial_missing) -> _T:
...
PathType = PathLike | str | Path | Traversable
