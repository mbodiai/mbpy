...
class SpawnBase(Protocol):
    ...
    def __init__(
        ...
        self.stdin: codecs.TextIO | Any = sys.stdin
...
class PopenSpawn(SpawnBase):
    def __init__(
        ...
        self.proc = subprocess.Popen(cmd, **kwargs)  # noqa: S603
        self.pid = self.proc.pid
        self.closed = False
        self._buf = self.string_type()
    ...
    def read_nonblocking(self, size, timeout):  # noqa: ANN201
        buf = self._buf
        if self._read_reached_eof:
            # We have already finished reading. Use up any buffered data,
            # then raise EOF
...
def socket_spawn(*args, **kwargs) -> SocketSpawn:
    return SocketSpawn(*args, **kwargs)
