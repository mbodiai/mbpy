...
class ParseInterruptedError(Exception):...
...
class _DictSAXHandler:
    ...
    def _build_name(self, full_name: str):
    ...
    def attrs_to_dict(self, attrs):
        if isinstance(attrs, dict):
            return attrs
        return self.dict_constructor(zip(attrs[0::2], attrs[1::2], strict=False))
...
def _emit(
    ...
    for index, v in enumerate(value):
        ...
        if v is None:
            ...
            v = {expand_iter: v,} if expand_iter and hasattr(v, "__iter__") else str(v)
        if isinstance(v, str):
            v = {cdata_key: v,}
        cdata = None
        attrs = {}
...
def unparse(input_dict, output=None, encoding="utf-8", full_document=True, short_empty_elements=False, **kwargs):
    r"""Emit an XML document for the given `input_dict` (reverse of `parse`).
    ...
    if output is None:
        output = StringIO()
        must_return = True
    content_handler = XMLGenerator(output, encoding, True) if short_empty_elements else XMLGenerator(output, encoding)
    if full_document:
    ...
    if must_return:
        ...
        try:  # pragma no cover  # noqa: SIM105
...
if __name__ == "__main__":  # pragma: no cover
    ...
    def handle_item(path, item) -> bool:
