...
# main function
def visualize_graphml(G: nx.Graph, html_path, port=8000):
    json_data = graphml_to_json(G)
    html_dir = Path(html_path).resolve().parent
    html_dir.mkdir(parents=True, exist_ok=True)
    
    json_path = Path(html_dir) / "graph_json.js"
...
if __name__ == "__main__":
    ...
    g = build_dependency_graph(".", include_site_packages=False)
    html_path = "view/view_graph.html"
    visualize_graphml(g.graph, html_path, 11236)